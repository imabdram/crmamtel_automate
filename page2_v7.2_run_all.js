...
// =========================================
// NAVIGATION HELPER - Navigate to Provisioning
// =========================================
async function navigateToProvisioning() {
  const wait = (ms) => new Promise(res => setTimeout(res, ms));
  
  console.log("🧭 Navigating to Provisioning...");
  
  // Step 1.1: Click the back arrow
  console.log("  📍 Step 1.1: Looking for back arrow...");
  const backArrow = document.querySelector('span.material-icons.back-icon');
  if (backArrow) {
    backArrow.click();
    console.log("  ✅ Back arrow clicked");
    await wait(800); // Wait for navigation
  } else {
    console.warn("  ⚠️ Back arrow not found - might already be on main page");
  }
  
  // Step 1.2: Click Provisioning toggle button
  console.log("  📍 Step 1.2: Looking for Provisioning toggle button...");
  const provisioningToggle = document.querySelector('button.btn-toggle.btn-menu[data-bs-toggle="collapse"][data-bs-target="#provisioning-collapse"]');
  if (provisioningToggle) {
    provisioningToggle.click();
    console.log("  ✅ Provisioning toggle clicked");
    await wait(1000); // Wait for collapse/expand animation
  } else {
    console.warn("  ⚠️ Provisioning toggle button not found");
  }
  
  // Step 1.3: Click Provisioning link
  console.log("  📍 Step 1.3: Looking for Provisioning link...");
  const provisioningLink = document.querySelector('a[href="#/provisioning"].link-dark.rounded');
  if (provisioningLink) {
    provisioningLink.click();
    console.log("  ✅ Provisioning link clicked");
    await wait(1500); // Wait for page to load
  } else {
    // Fallback: try without the rounded class
    const fallbackLink = document.querySelector('a[href="#/provisioning"]');
    if (fallbackLink) {
      fallbackLink.click();
      console.log("  ✅ Provisioning link clicked (fallback selector)");
      await wait(1500);
    } else {
      console.error("  ❌ Provisioning link not found");
      return false;
    }
  }
  
  console.log("✅ Navigation to Provisioning complete!");
  return true;
}
...
async function runFullActivation() {
  ...
  try {
    // Step 1: Page 1 (now includes navigation)
    console.log("📝 Step 1/3: Running Page 1...");
    await page1();  // <-- Added 'await' here
    ...
    // --- NEW: Navigate back to Provisioning after completion ---
    console.log("🔄 Navigating back to Provisioning for next activation...");
    await navigateToProvisioning();
...
// =========================================
// SIMPLE REPEAT FUNCTION - go(n)
// =========================================

async function go(repeatCount = 1) {
  // If no number provided, run once
  if (repeatCount === 1) {
    console.log("🚀 Running single activation...");
    return await runFullActivation();
  }
  
  // Run multiple times
  console.log("=".repeat(60));
  console.log(`🔄 Running ${repeatCount} activations...`);
  console.log("=".repeat(60));
  ...
  for (let i = 0; i < repeatCount; i++) {
    ...
    console.log(`\n📱 Activation ${currentRun}/${repeatCount}`);
    ...
    try {
      ...
      if (result === true) {
        successful++;
        console.log(`✅ Activation ${currentRun} completed successfully`);
      } else {
        failed++;
        console.log(`❌ Activation ${currentRun} failed`);
      }
      
      // Wait 3 seconds between runs (except after the last one)
      if (i < repeatCount - 1) {
        console.log(`⏳ Waiting 3 seconds before next activation...`);
        await new Promise(res => setTimeout(res, 3000));
      }
      
    } catch (error) {
      failed++;
      console.error(`❌ Activation ${currentRun} error:`, error);
    }
  }
  ...
  console.log(`📋 Total: ${repeatCount}`);
  console.log("=".repeat(60));
  
  return { successful, failed, total: repeatCount };
}

// Keep the original single-run behavior
// If you call go() without a number, it runs once
// If you call go(5), it runs 5 times
